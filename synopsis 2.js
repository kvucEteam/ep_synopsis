var info_Array = ["Her er noget info om indledningen", "Her er noget info om problemformulering", "Du skal sørge for at tilgodese begge fags taksonomiske forståelse. <br>Hvert underspørgsmål skal inddrage faglige begreber og indhold fra mindst et af fagene. <br/>Begge fag skal fremgå i dine underspørgsmål.", "Du skal skrive hvordan du anvender begge fags metoder, men du behøver ikke begrunde hvorfor.<br>Overvej hvilken fremgangsmåde (altså hvilke faglige metoder) du benytter til at besvare din problemformulering.<br/>Der kan indgå teorier fra de anvendte fag."];
...
function template() {
    ...
    HTML += '<h3 class="info_hover">Indledning <span class="glyphicon glyphicon-info-sign"></span></h3>';
    ...
    ++titelIndex;
    HTML += '<h2> Problemformulering og underspørgsmål</h2>';
    HTML += '<div class="textindent">';
    HTML += '<h3 class="info_hover">Problemformulering <span class="glyphicon glyphicon-info-sign"></span></h3>';
    ...
    HTML += '<h3 class="info_hover">Underspørgsmål <span class="glyphicon glyphicon-info-sign"></span></h3>';
    ...
    ++titelIndex;
    HTML += '<h2> Besvarelse af underspørgsmål</h2>';
    HTML += '<div class="textindent">';
    HTML += 'Du skal besvare dine underspørgsmål ved at anvede begge fags metoder og teori.';
    HTML += '</div>';
    ...
    ++titelIndex;
    HTML += '<h2 class="info_hover"> Materialer <span class="glyphicon glyphicon-info-sign"></span></h2>';
...
*/
    ...
    HTML += '</div>';
    HTML += '<span id="addNewBibOptional" class="btn btn-info"><span class="glyphicons glyphicons-plus"></span>Tilføj materiale</span>';
    ...
    $(".info_hover").on('mouseover touchstart', function() {
        var identifier = $(this).index(".info_hover");
        //alert(jsonData.templateData.introtekster[0])s;
        //microhint($(this), jsonData.templateData.introtekster[identifier]);
        microhint($(this), info_Array[identifier]); //jsonData.templateData.introtekster[identifier]);
        $(this).mouseout(function() { $(".microhint").remove() });
...
function saveJsonData() {
    jsonData.templateData = { theme: $('.studentTheme').val(), fagkombination: $('.studentFag').val(), introduction: $('#introduction').val(), problemformulation: $('#problemformulation').val(), subQuestions: [], conclusion: $('#conclusion').val(), metode: $('#metode').val(), bibliography: { mandatory: [], optional: [] } };
...
function wordTemplate() {
    ...
    HTML += '<h3>Metodiske overvejelser</h3>';
    HTML += '<p>' + obj.metode + '</p>';
...
*/
    ...
    HTML += '<h3 style="margin: 0px; padding: 0px 0px 10px 0px;">Tjekspørgsmål til problemformuleringen:</h3><p><b>Rød tråd:</b> Hænger hoved- og underspørgsmål sammen? Dvs. besvares hovedspørgsmålet med underspørgsmålene? Og er der en sammenhæng mellem underspørgsmålene?</p>';
    HTML += '<p><b>Taksonomi:</b> Lægger hovedspørgsmålet op til undersøgelse, diskussion og vurdering (ikke kun redegørelse)?</p>';
    HTML += '<p><b>Tværfaglighed:</b> Kan teori og metode fra begge fag inddrages i besvarelsen af problemformuleringen?</p>';
...
function dataHasBeenEntered() {
    ...
    if (JT.theme.length > 0) {
        return true;
    }
    if (JT.introduction.length > 0) {
        return true;
    }
    if (JT.problemformulation.length > 0) {
        return true;
    }
    if (JT.conclusion.length > 0) {
        return true;
    }
    ...
    for (var n in JT.bibliography.mandatory) {
        if (JT.bibliography.mandatory[n].source.length > 0) {
            return true;
        }
        if (JT.bibliography.mandatory[n].characterization.length > 0) {
            return true;
        }
    }

    for (var n in JT.bibliography.optional) {
        if (JT.bibliography.optional[n].source.length > 0) {
            return true;
        }
        if (JT.bibliography.optional[n].characterization.length > 0) {
            return true;
        }
    }
